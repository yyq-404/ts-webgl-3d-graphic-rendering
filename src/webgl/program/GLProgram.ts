...
export class GLProgram {
    ...
    loadShaders(vs: string, fs: string): void {
        ...
        if (!GLHelper.linkProgram(this.gl, this.program, this.vsShader, this.fsShader, this.progromBeforeLink.bind(this), this.progromAfterLink.bind(this),)) {
    ...
    setVector2(name: string, vec2: Vector2): boolean {
        const loc: WebGLUniformLocation | null = this.getUniformLocation(name);
        if (loc) {
    ...
    setVector3(name: string, vec3: Vector3): boolean {
        const loc: WebGLUniformLocation | null = this.getUniformLocation(name);
        if (loc) {
    ...
    setVector4(name: string, vec4: Vector4): boolean {
        const loc: WebGLUniformLocation | null = this.getUniformLocation(name);
        if (loc) {
    ...
    setQuat(name: string, quat: Quaternion): boolean {
        const loc: WebGLUniformLocation | null = this.getUniformLocation(name);
        if (loc) {
    ...
    setMatrix3(name: string, mat: Matrix4): boolean {
        const loc: WebGLUniformLocation | null = this.getUniformLocation(name);
        if (loc) {
    ...
    setMatrix4(name: string, mat: Matrix4): boolean {
        const loc: WebGLUniformLocation | null = this.getUniformLocation(name);
        if (loc) {
    ...
    loadModeViewMatrix(mat: Matrix4): boolean {
